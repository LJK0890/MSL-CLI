...
internal class GlobalManager : IDisposable
{
    ...
    public void PrintConfig()
    {
        if (appConfig == null)
        {
            Print("Global/Config", LogLevel.INFO, "配置未加载", includeTimestamp: true);
            return;
        }
        Print("Global/Config", LogLevel.INFO, "EnableAI: " + appConfig.EnableAI, includeTimestamp: true);
        Print("Global/Config", LogLevel.INFO, "AIConfigs:", includeTimestamp: true);
        foreach (var kvp in appConfig.AIConfigs)
        {
            string keyDisplay = string.IsNullOrEmpty(kvp.Value.ApiKey) ? "(empty)" : "****";
            Print("Global/AI", LogLevel.INFO, $" {kvp.Key}:", includeTimestamp: true);
            Print("Global/AI", LogLevel.INFO, $" Url={kvp.Value.Url}", includeTimestamp: true);
            Print("Global/AI", LogLevel.INFO, $" Model={kvp.Value.Model}", includeTimestamp: true);
            Print("Global/AI", LogLevel.INFO, $" ApiKey={keyDisplay}", includeTimestamp: true);
            Print("Global/AI", LogLevel.INFO, $" UseApiKeyEnv={kvp.Value.UseApiKeyEnv}", includeTimestamp: true);
            Print("Global/AI", LogLevel.INFO, $" ApiKeyEnv={kvp.Value.ApiKeyEnv}", includeTimestamp: true);
        }
        Print("Global/Config", LogLevel.INFO, "ServerPaths:", includeTimestamp: true);
        foreach (var kvp in appConfig.ServerPaths)
        {
            Print("Global/Config", LogLevel.INFO, $" {kvp.Key}: {kvp.Value}", includeTimestamp: true);
        }
    }

    public async Task MainLoop()
    {
        await serverManagers["yz"].StartAsync();
        while (true)
        {
